...
def generate_base_data(base_features, base_labels):
    base_data = {}
    # logging.info(f"[generate_base_data] set(labels): {set(labels)}")
    # logging.info(f"[generate_base_data] set(base_labels): {set(base_labels)}")
...
def DC_aug(args, features, labels, support_data, support_label, base_data, base_means, base_cov):
    ...
    for i in range(sample_num):
        ...
        cov = np.cov(tmp.T)
        # logging.info(f'[DC_aug] mean: {mean}')
        # logging.info(f'[DC_aug] tmp_mean: {tmp_mean}')
        # logging.info(f'[DC_aug] mean == tmp_mean: {all(mean==tmp_mean)}')
        # logging.info(f'[DC_aug] cov == tmp_cov: {(cov==tmp_cov).all()}')
    ...
    # sampled_data = Tukey_Transform(sampled_data, info="DC_sample_data")
...
1300 + 98 --> 84.58%
========================================================================
python sample_tools/DC_v3.py -p 1 --val -un --gpu -r kmeans_sample_ids/ImageNet_1300_sample_ids.npy
---------------------
1300 + 83 --> 81.44%
1300 + __ --> 81.46%
1300 + 90 --> 81.06%
